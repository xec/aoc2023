...
const lineRegex = /^Card.*: (.*)\|(.*)$/;

let score = 0;

exampleInput.split("\n").forEach((line) => {
  const linesplit = line.match(lineRegex);
  const winningNumbers = linesplit[1].split(" ").filter(Boolean);
  const scratchedNumbers = linesplit[2].split(" ").filter(Boolean);
  const matchedNumbers = scratchedNumbers.filter((s) =>
    winningNumbers.includes(s)
  );
  const lineScore = matchedNumbers.reduce((acc) => {
    if (!acc) return 1;
    return acc * 2;
  }, 0);
  //   console.log(
